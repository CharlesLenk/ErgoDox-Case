        ...
        union() {
            difference() {
                ...
                translate([0, 0, bottomScrewPostHeight + plateThickness + 0.001]) {
                    case_skirt(wallThickness/2, wallThickness + 0.001, 10);
                }                         
            }
            bottom_screw_posts();
        }
    ...
    union() {
        difference() {
            union() {
                ...
                difference() {
                    ...
                    translate([0, 0, portHeight - topScrewPostHeight + wallThickness/2]) {
                        case_skirt(-2 * wallThickness, wallThickness - 0.001, topScrewPostHeight);
                    }
                }
            ...
            translate([0, 0, portHeight - wallThickness/2 - 1]) {
                case_skirt(- 0.001,  wallThickness/2, wallThickness/2 + 1);
            }
            translate([-1, -1, 0]) {
                cube([pcbCircuitXDimension + 1.001, baseDimensionY + 2, 20]); 
            }  
            screw_holes(frontCoordinates, height - 1);
            key_cutouts();    
        }                   
    ...
    difference() {
        union() {
            difference() {
                ...
                top_cover_rectangle(plateThickness, wallThickness, 1);
                translate([-1, -1, height - topScrewPostHeight - plateThickness - 10 - 0.001]) {
                    cube([500, 500, 10]); 
                }
            ...
            for(coordinate = backCoordinates) {
                top_cover_screw_post(coordinate);
            }
        }
        ...
        screw_holes(backCoordinates, height + circuitBumpHeight - 1);
...
module case_shell(height) {
    difference() {
        ...
        case_base(height, plateThickness, wallThickness);
    }
}    
...
module case_base(height, height_offset, edge_offset) {
    union() {
        ...
        translate([thumbOffsetX, thumbOffsetY, height_offset]){
            rotate(-25) {
                translate([-thumbDim/2 + edge_offset, -1.5 * thumbDim + edge_offset, 0]){
                    roundedCube(thumbDim - 2 * edge_offset, 2*thumbDim - 2 * edge_offset, height - 2 * height_offset, edgeDiameter, diameter);
                }
            }
        }
    }
...
module top_cover_rectangle(height_offset, edge_offset, xOffset) {
    translate([edge_offset, edge_offset, height_offset]) {
        roundedCube(pcbCircuitXDimension - edge_offset - xOffset, baseDimensionY - 2 * edge_offset, height + circuitBumpHeight - 2 * height_offset, edgeDiameter, diameter);
    }
}

module case_rectangle(height, height_offset, edge_offset) {
     translate([edge_offset, edge_offset, height_offset]) {
        roundedCube(baseDimensionX - 2 * edge_offset, baseDimensionY - 2 * edge_offset, height - 2 * height_offset, edgeDiameter, diameter, true);
    }
}
...
module key_cutouts() {
    translate([baseX + middleColumnsShift, baseY + keyGap/2, 0]) {    
        key_cutout(2*keyDist + outerColumnShift, - 3 * keyDist);
        for(i = [-2 : 2]) {
            if(i < 2) {
                key_cutout(i * keyDist + outerColumnShift, -outerRowKeyShift - 3 * keyDist);
                key_cutout(i * keyDist + innerColumnShift, 2 * keyDist);
            }
            key_cutout(i * keyDist + outerColumnShift, -2 * keyDist);
            key_cutout(i * keyDist + middleColumnsShift, -keyDist);
            key_cutout(i * keyDist, 0);
            key_cutout(i * keyDist + middleColumnsShift, keyDist);        
        }              
        innerDownShift1 = 4.7625;
        innerDownShift2 = innerDownShift1 + 9.5278;
        translate([innerColumnShift, 0, 0]) {        
            key_cutout(-2 * keyDist, 3 * keyDist);
            key_cutout(-keyDist + innerDownShift1, 3 * keyDist);
            key_cutout(innerDownShift2, 3 * keyDist);
        }
    }
    translate([thumbOffsetX, thumbOffsetY, 0]){
        rotate(-25) {
            translate([-keyCutDim/2 , -keyCutDim/2 , 0]) {
                for(i = [-1 : 1]) {
                    key_cutout(i * keyDist, keyDist);
                }
                key_cutout(-keyDist, 0);
                key_cutout(keyDist/2, 0);
                key_cutout(keyDist/2, -keyDist);
            }
        }
    }
...
module top_cover_screw_post(coordinate) {
...
module roundedCube(x, y, z, eDia = 0.001, sDia = 0.001, useSpecialCorner = false) {
    hull() {
        translate([x - eDia/2, y - eDia/2, 0]) {
            rounded_cylinder_2(z, eDia, sDia/2);
        }
        ...
        if(useSpecialCorner) {
            ...
            translate([x - specialCornerDia/2, specialCornerDia/2, 0]) {
                rounded_cylinder_2(z, specialCornerDia, sDia/2);
            }
        }
        else {
